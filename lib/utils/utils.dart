...
void showBottomMessage(BuildContext context, String message) {
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Container(
        margin: const EdgeInsets.all(5), // Marge sur tous les côtés de l'arrière-plan
        padding: const EdgeInsets.all(15), // Optionnel : vous pouvez ajouter un padding à l'intérieur du conteneur
        decoration: BoxDecoration(
          color: isDark ? Color(0xFFf1f1f1) : Color(0xFF3c3c3c),
          borderRadius: BorderRadius.circular(5), // Optionnel : pour arrondir les coins du conteneur
        ),
        child: Text(message, style: TextStyle(color: isDark ? Colors.black : Colors.white, fontSize: 15)),
      ),
      duration: Duration(seconds: 3),
      backgroundColor: Colors.transparent, // Rendre le fond du SnackBar transparent
    ),
  );
}

Future<Map<String, dynamic>?> searchPub(String pubSymbol, String issueTagNumber) async {
  ...
  if (await catalogFile.exists() && await mepsFile.exists()) {
    ...
    List<Map<String, dynamic>> publications = await catalog.rawQuery('''
      ...
      WHERE 
        pa.MepsLanguageId = ? AND LOWER(p.KeySymbol) = LOWER(?) AND p.IssueTagNumber = ?
      GROUP BY 
        p.Id
      LIMIT 1
