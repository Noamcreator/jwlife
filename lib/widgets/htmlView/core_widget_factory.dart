...
import 'package:flutter/material.dart'
    show CircularProgressIndicator, SelectableText, Tooltip;
...
class WidgetFactory extends WidgetFactoryResetter with AnchorWidgetFactory {
  ...
  Widget? buildText(
    ...
    // Pré-calculer autant de paramètres que possible
    ...
    return Builder(
      builder: (context) {
        // TODO: supprimer Builder quand ListView cessera de fournir son propre registraire
        ...
        // Créez un RichText pour le texte
        ...
        // Si un registraire de sélection est disponible, encapsulez dans MouseRegion
        if (selectionRegistrar != null) {
          built = MouseRegion(cursor: SystemMouseCursors.text, child: built);
        }

        // Utiliser SelectableText pour rendre le texte sélectionnable
        return SelectableText.rich(
          TextSpan(
            children: [
              // Ajouter le texte dans une liste de TextSpan
              if (text is TextSpan) text else TextSpan(text: text.toPlainText()), // Convertir InlineSpan en TextSpan
            ],
          ),
          style: DefaultTextStyle.of(context).style, // Utiliser le style de texte par défaut du contexte
          maxLines: maxLines,
          textAlign: textAlign,
        );
      },
  ...
  InlineSpan? buildSelectableTextSpan({
    List<InlineSpan>? children,
    GestureRecognizer? recognizer,
    TextStyle? style,
    String? text,
  }) {
    // Si le texte est vide, retournez un SelectableText vide
    ...
    // Crée un TextSpan avec les enfants et les styles donnés
    ...
    // Retourne un SelectableText avec le TextSpan créé
    return WidgetSpan(child: SelectableText.rich(textSpan));
  }
  ...
  /// Calls [DocumentHtmlWidget.onTapUrl] with [url].
  ///
  ...
  /// This also handles styling from [BuildOp]s and [DocumentHtmlWidget.customStylesBuilder].
  ...
  void reset(State state) {
    ...
    _widget = widget is DocumentHtmlWidget ? widget : null;
  }

  /// Resolves full URL with [DocumentHtmlWidget.baseUrl] if available.
...
class WidgetFactoryResetter {
  /// Called when the [DocumentHtmlWidget]'s state is disposed.
  @mustCallSuper
  void dispose() {}
