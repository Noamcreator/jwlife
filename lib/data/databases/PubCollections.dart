...
class PubCollections {
  ...
  Future<void> init() async {
    ...
    if (await pubCollections.exists()) {
  ...
  Future<void> fetchDownloadPublications() async {
    ...
    List<Map<String, dynamic>> result = await _database.rawQuery('''
        SELECT DISTINCT
          ...
          PublicationIssueProperty.CoverTitle,
          PublicationIssueProperty.UndatedSymbol,
          ...
          (SELECT Image.Path 
     FROM Image 
     WHERE Image.Type = 't' AND Image.PublicationId = Publication.PublicationId 
     ORDER BY Image.Width DESC, Image.Height DESC 
     LIMIT 1) AS ImageSqr,
    (SELECT Image.Path 
     FROM Image 
     WHERE Image.Width = 1200 
       AND Image.Height = 600 
       AND Image.Type = 'lsr' 
       AND Image.PublicationId = Publication.PublicationId 
     LIMIT 1) AS ImageLsr
        FROM Publication
        LEFT JOIN PublicationAttribute ON PublicationAttribute.PublicationId = Publication.PublicationId
        LEFT JOIN PublicationIssueProperty ON PublicationIssueProperty.PublicationId = Publication.PublicationId
        LEFT JOIN meps.Language ON Publication.MepsLanguageId = meps.Language.LanguageId
      ''');
  ...
  Future<Publication?> getDocumentFromMepsDocumentId(int mepsDocId, int currentLanguageId) async {
    ...
    return publications.firstWhere((element) => element.keySymbol == result.first['KeySymbol'] && element.issueTagNumber == result.first['IssueTagNumber'] && element.mepsLanguage.id == result.first['MepsLanguageId']);
  }
}
