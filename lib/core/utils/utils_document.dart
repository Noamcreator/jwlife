...
void showChapterView(BuildContext context, String keySymbol, int bookNumber, int chapterNumber, int currentLanguageId) async {
  ...
  if (bible != null) {
    showPage(context, DocumentView.bible(bible: bible, book: bookNumber, chapter: chapterNumber));
  }
  else {
    if(await hasInternetConnection()) {
...
String createHtmlContent(String html, String articleClasses, Publication publication, bool hasAppBar) {
  ...
  document.querySelector('.content-wrapper').style.paddingTop = paddingTop;
  document.querySelector('.content-wrapper').style.paddingBottom = '50px';
  ...
  document.body.addEventListener('click', (event) => {
    ...
    if (event.target && event.target.classList.contains('fn')) {
      ...
      window.flutter_inappwebview.callHandler('openFootNote', fnid);
    }
  });
...
Future<Map<String, dynamic>?> showBookmarkDialog(BuildContext context, Publication publication, {int? mepsDocumentId, String? title, String? snippet, int? blockType, int? blockIdentifier}) async {
                              ...
                              InkWell(
                                onTap: () async {
                                  if (bookmark.isNotEmpty) {
                                    ...
                                    Map<String, dynamic> bookmark = await JwLifeApp.userdata.addBookmark(publication, mepsDocumentId, title!, snippet!, index, blockType!, blockIdentifier);
                                    setState(() {
