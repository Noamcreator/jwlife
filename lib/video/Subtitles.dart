...
class Subtitles {
  ...
  String toString() {
    ...
    final htmlTagPattern = RegExp(r'<[^>]*>'); // correspond à tout texte entre '<' et '>'
    ...
    for (Subtitle subtitle in subtitles) {
      ...
      // Vérifie si le texte se termine par un point, si oui, ajoute la phrase à result
      if (subtitle.text.lastIndexOf('.') == subtitle.text.length - 1 || subtitle.text.lastIndexOf('?') == subtitle.text.length - 1 || subtitle.text.lastIndexOf('!') == subtitle.text.length - 1) {
        result += '$phrase\n';
        phrase = '';
      }
    }
