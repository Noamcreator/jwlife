...
class RealmMediaHandler {
  static Future<void> convertMediaJsonToRealm(var bodyBytes) async {
    ...
    String languageCode = '';
    Set<String> existingLanguageSymbols = Set.from(JwLifeApp.library.all<Language>().map((lang) => lang.symbol));
    Set<String> existingMediaItemKeys = Set.from(JwLifeApp.library.all<MediaItem>().map((item) => item.naturalKey));
    Map<String, Set<String>> existingCategoryKeysByLanguage = {};

    // Initialiser un ensemble pour chaque langue
    for (var lang in JwLifeApp.library.all<Category>()) {
    ...
    for (var json in jsonList) {
      ...
      if (json['type'] == 'category') {
        ...
        var existingCategory = JwLifeApp.library.all<Category>().query("key == '${data['key']}' AND language == '$languageCode'").firstOrNull;
        if (existingCategory != null) {
          JwLifeApp.library.write(() {
            JwLifeApp.library.delete(existingCategory);
          });
        }
    ...
    JwLifeApp.library.write(() {
      JwLifeApp.library.addAll(languagesToAdd);
      JwLifeApp.library.addAll(categoriesToAdd);
      JwLifeApp.library.addAll(mediaItemsToAdd);
    });
  }
