...
class _DocumentView extends State<DocumentView> with SingleTickerProviderStateMixin {
  int _documentId = 0;
  Database? _database;
...
""");
  ...
  Future<void> switchImageMode() async {
    if (_isImageMode) {
      _controller.loadData(data: _htmlContent, baseUrl: WebUri('file:///android_asset/flutter_assets/assets/webapp/'));
...
''';
  ...
  Future<void> fetchDocument() async {
    ...
    ''';
      ...
      String decodedHtml = await decodeBlobContentWithHash(
      ...
      ''';
        ...
        final contentBlob = document['Content'] as Uint8List;
        decodedHtml = await decodeBlobContentWithHash(
          contentBlob: contentBlob,
          hashPublication: widget.publication['Hash'],
        );
