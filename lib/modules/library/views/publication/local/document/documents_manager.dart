...
class DocumentsManager {
  ...
  String createReaderHtmlShell(DocumentView widget) {
      ...
      <body class="${webViewData.theme}">
        ...
        <script>
          ...
          async function loadIndexPage(index) {
            ...
            // Copier le contenu de la page dans la loupe
            magnifierContent.innerHTML = pageCenter.innerHTML;
            
            // Appliquer exactement les mêmes dimensions que pageCenter
            const pageCenterStyles = window.getComputedStyle(pageCenter);
            magnifierContent.style.width = pageCenterStyles.width;
            magnifierContent.style.height = pageCenterStyles.height;
            magnifierContent.style.padding = pageCenterStyles.padding;
            magnifierContent.style.margin = pageCenterStyles.margin;
            magnifierContent.style.fontSize = pageCenterStyles.fontSize;
            magnifierContent.style.lineHeight = pageCenterStyles.lineHeight;
            magnifierContent.style.textAlign = pageCenterStyles.textAlign;
            magnifierContent.style.whiteSpace = pageCenterStyles.whiteSpace;
          ...
          function showToolbar(paragraphs, pid, selector, hasAudio, type) {
            ...
            if (type === 'verse') {
              buttons = [
                ['&#xE65B;', () => callHandler('showVerse', { paragraphId: pid })],
                ['&#xE681;', () => addNote(paragraph, pid, true, '')],
                ['&#xE61F;', () => callHandler('showOtherTranslations', { paragraphId: pid })],
                ['&#xE62A;', () => callHandler('bookmark', { snippet: paragraph.innerText, paragraphId: pid, isBible: isBible() })],
                ['&#xE651;', () => callHandler('copyText', { text: paragraph.innerText })],
                ['&#xE620;', () => callHandler('searchVerse', { query: pid })],
                ['&#xE6A3;', () => callHandler('share', { paragraphId: pid, isBible: true })],
              ];
            } else {
              buttons = [
                ['&#xE681;', () => addNote(paragraph, pid, false, '')],
                ['&#xE62A;', () => callHandler('bookmark', { snippet: paragraph.innerText, paragraphId: pid })],
                ['&#xE6A3;', () => callHandler('share', { paragraphId: pid, isBible: false })],
                ['&#xE651;', () => callHandler('copyText', { text: paragraph.innerText })],
              ];
            }
            ...
            if (hasAudio) {
              toolbar.appendChild(createToolbarButton('&#xE65E;', () => callHandler('playAudio', { paragraphId: pid })));
            }
          }
          ...
          const verseElementsCache = new Map();
          
          function whenClickOnParagraph(paragraphs, selector, idAttr, classFilter) {
            const current = paragraphs[0];
            if (!current) {
              closeToolbar();
              return;
            }
            ...
            if (classFilter === 'paragraph') {
              const matchedElement = current.closest(`[\${idAttr}]`);
            ...
            // Extraction d'ID optimisée sans split/join coûteux
            if (vid.startsWith('v') && current.classList.contains('v')) {
              const thirdDash = vid.indexOf('-', vid.indexOf('-') + 1);
              if (thirdDash !== -1) {
                const fourthDash = vid.indexOf('-', thirdDash + 1);
                if (fourthDash !== -1) {
                  vid = vid.substring(0, fourthDash);
                }
              }
            }
          
            // Cache avec clé composite incluant l'index de page
            const cacheKey = `\${vid}_\${currentIndex}`;
            let verseElements = verseElementsCache.get(cacheKey);
            
            if (!verseElements) {
              // Requête DOM avec sélecteur optimisé
              verseElements = Array.from(pageCenter.querySelectorAll(`span.v[id^="\${vid}"]`));
          
              // Tri optimisé sans split répétés
              verseElements.sort((a, b) => {
                const aId = a.getAttribute('id');
                const bId = b.getAttribute('id');
                const aLast = parseInt(aId.substring(aId.lastIndexOf('-') + 1)) || 0;
                const bLast = parseInt(bId.substring(bId.lastIndexOf('-') + 1)) || 0;
                return aLast - bLast;
              });
          
              // Mise en cache
              verseElementsCache.set(cacheKey, verseElements);
            }
          
            // Log conditionnel pour éviter les appels inutiles
            if (console?.log) {
              console.log('Éléments du verset trouvés:', verseElements.map(el => el.getAttribute('id')));
            }
          
            // Condition ternaire pour réduire les branches
            verseElements.length > 0 ? 
              showToolbar(verseElements, vid, selector, false, classFilter) : 
              closeToolbar();
          }
            ...
            pageCenter.querySelectorAll(selector).forEach(p => {
              ...
              if (bibleMode && id?.startsWith('v')) {
                ...
                if (segments.length >= 3) {
                  id = segments[2]; // Garder seulement le 3e segment
                }
              }
          
              const idKey = `\${blockType}-\${id}`;
              ...
              matchingHighlights.forEach(h => {
                addHighlight(p, h.BlockType, h.Identifier, h.StartToken, h.EndToken, h.UserMarkGuid, h.ColorIndex);
              });
          
              const matchingNotes = notesMap.get(id) || [];
              ...
              matchingNotes.forEach(note => {
                ...
                processedNoteGuids.add(note.Guid); // Marquer comme ajouté
              });
            });
          ...
          function removeBookmark(blockIdentifier, slot) {
            ...
            if (bookmark.getAttribute('slot') === slot) {
              bookmark.remove();
            }
          }
          ...
          function addHighlight(target, blockType, blockIdentifier, startToken, endToken, guid, colorIndex) {
            ...
            // Liste complète des tokens à prendre en compte, peu importe leur profondeur
            const tokens = Array.from(target.querySelectorAll('.word, .punctuation, .escape'));
          
            // On veut seulement les mots et ponctuations pour le slicing
            const wordAndPunctTokens = tokens.filter(token => token.classList.contains('word') || token.classList.contains('punctuation'));
            ...
            selectedTokens.forEach((token, index) => {
              ...
              // Trouver l'élément escape qui vient juste après ce token dans le DOM complet
              const tokenIndexInAll = tokens.indexOf(token);
              const next = tokens[tokenIndexInAll + 1];
          ...
          pageCenter.addEventListener('click', (event) => {
            ...
            if (isBible()) {
              whenClickOnParagraph(targets, '.v', 'id', 'verse');
            } else {
              whenClickOnParagraph(targets, '[data-pid]', 'data-pid', 'paragraph');
            }
          }, { passive: true });
          ...
          pageCenter.addEventListener('touchend', (event) => {
            ...
            if (isLongPressing) {
              if(isLongTouchFix) {
                 firstLongPressTarget.setAttribute('data-highlight-id', currentGuid);
              } 
              onLongPressEnd();
            }
            isLongTouchFix = false;
          ...
          // Fonction updateTempHighlight optimisée
          function updateTempHighlight() {
            if (!firstLongPressTarget || !lastLongPressTarget) return;
          
            const paragraphInfo = getTheFirstTargetParagraph(firstLongPressTarget);
            if (!paragraphInfo) return;
            ...
            const startIndex = wordAndPunctTokens.indexOf(firstLongPressTarget);
            const endIndex = wordAndPunctTokens.indexOf(lastLongPressTarget);
            ...
            const selectedTokens = wordAndPunctTokens.slice(from, to + 1);
          
            // Optimisation avec requestAnimationFrame pour les modifications DOM
            requestAnimationFrame(() => {
              selectedTokens.forEach((token, index) => {
                ...
                // Optimisation de la recherche d'éléments suivants
                const tokenIndexInAll = allTokens.indexOf(token);
                const next = allTokens[tokenIndexInAll + 1];
          
                if (next?.classList.contains('escape') && index !== selectedTokens.length - 1) {
          ...
          // Fonction onLongPressEnd optimisée avec gestion d'erreurs
          async function onLongPressEnd() {
            ...
            try {
              const paragraphInfo = getTheFirstTargetParagraph(firstTarget);
              if (!paragraphInfo) return;
              ...
              const normalizedStartToken = Math.min(startIndex, endIndex);
              const normalizedEndToken = Math.max(startIndex, endIndex);
              const selectedWords = allWords.slice(normalizedStartToken, normalizedEndToken + 1);
              
              // Traitement des highlights existants
              for (const word of selectedWords) {
                const highlightId = word.getAttribute('data-highlight-id');
                if (highlightId && highlightId !== currentGuid) {
                  await window.flutter_inappwebview.callHandler('removeHighlight', { guid: highlightId });
                  
                  const allHighlights = allWords.filter(w => w.getAttribute('data-highlight-id') === highlightId);
                  const startIndexWord = allWords.indexOf(allHighlights[0]);
                  const endIndexWord = allWords.indexOf(allHighlights[allHighlights.length - 1]) + 1;
                  
                  // Suppression optimisée des classes
                  allHighlights.forEach(highlight => {
                    highlightClasses.forEach(cls => highlight.classList.remove(cls));
                    highlight.removeAttribute('data-highlight-id');
                  });
                  
                  addHighlight(paragraph, blockType, id, startIndexWord, endIndexWord, currentGuid, highlightColorIndex);
                }
              }
          
              // Sauvegarde en base de données
              await window.flutter_inappwebview.callHandler('addHighlight', {
                blockType,
                identifier: id,
              ...
              console.error('Error in onLongPressEnd:', error);
            }
          }
