    ...
    if (language.isEmpty || language == lang.vernacular) {
      setState(() {
        ...
        filteredAudios = widget.category.media.map((key) {
          return JwLifeApp.library.all<MediaItem>().query("naturalKey == '$key'").first;
        }).toList();
      });
    }
    else {
      String key = widget.category.key!;
      List<Category> categories = JwLifeApp.library.all<Category>().query("key == '$key' AND language == '$languageCode'").toList();
    ...
    if (query.isEmpty) {
      ...
      setState(() {
        ...
        filteredAudios = widget.category.media.where((audio) {
          // On récupère l'objet MediaItem correspondant à la clé
          MediaItem mediaItem = JwLifeApp.library.all<MediaItem>().query("naturalKey == '$audio'").first;
          ...
          return JwLifeApp.library.all<MediaItem>().query("naturalKey == '$audio'").first;
        }).toList();
      });
    }
  }
