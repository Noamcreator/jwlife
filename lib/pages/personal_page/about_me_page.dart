...
class _AboutMePageState extends State<AboutMePage> {
  ...
  Future<void> _loadMeData() async {
    _me = await JwLifeApp.userdata.getMe();
    if (_me.isNotEmpty) {
      setState(() {
        ...
        _genderId = _me['GenderId'] ?? 1; // Changez pour utiliser GenderId
        _localAssemblyId = _me['KingdomHallId'] ?? -1;
        _groupId = _me['GroupId'] ?? -1;
        _roleId = _me['RoleId'] ?? 3; // Changez pour utiliser RoleId
        _baptismDateController.text = _me['BaptemDate'] ?? '';
        _lastVisitDateController.text = _me['LastVisitDate'] ?? '';
        _pioneerId = _me['PioneerId'] ?? 0; // Changez pour utiliser PioneerId
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDropdown('Genre', _genderOptions, _genderId, (value) {
                    setState(() {
                      _genderId = value['id']; // Changez pour utiliser l'ID
                    });
                  }),
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDropdown('Rôle', _roleOptions, _roleId, (value) {
                    setState(() {
                      _roleId = value['id']; // Changez pour utiliser l'ID
                    });
                  }),
                  ...
                  _buildDropdown('Pionnier', _pioneerOptions, _pioneerId, (value) {
                    setState(() {
                      _pioneerId = value['id']; // Changez pour utiliser l'ID
                    });
                  }),
                  Row(
                    children: [
  ...
  Widget _buildDropdownWithSearch(String label, List<Map<String, dynamic>> options, int selectedId, Function(Map<String, dynamic>) onChanged, TextEditingController controller) {
    // Vérifiez si la liste des options est vide
    if (options.isEmpty) {
      return Text("Aucune option disponible");
    }

    // Recherchez l'option correspondante ou utilisez une valeur par défaut
    Map<String, dynamic> selectedOption = options.firstWhere(
            (option) => option['id'] == selectedId,
        orElse: () => options.first // Choisissez la première option par défaut
    );
